...
# Pure JS version of SetupScreen. No backticks for HTML.
setup_screen_code = """    // ── Setup Screen (Pure JS - No Template Literals) ─────────────
    function SetupScreen({ onJoin }) {
      const h = React.createElement; // Alias for brevity
      ...
      const timer = cd ? h('div', { style: timerContainerStyle },
        [["D", cd.d], ["H", cd.h], ["M", cd.m], ["S", cd.s]].map(([l, v]) => 
          h('div', { key: l, style: timerBoxStyle },
            h('div', { className: "bebas", style: timerValStyle }, String(v).padStart(2, "0")),
            h('div', { style: timerLabelStyle }, l)
          )
        )
      ) : null;

      const title = h(React.Fragment, null,
        h('div', { style: dateStyle }, "FEB 8 • 2026"),
        h('h1', { className: "bebas", style: titleStyle },
          "SUPER BOWL ", h('span', { style: lxStyle }, "LX")
        )
      );

      const matchup = h('div', { style: matchupContainerStyle },
        h('div', { style: teamBoxStyle },
          h('img', { src: "https://a.espncdn.com/i/teamlogos/nfl/500/sea.png", style: logoStyle(SEA), alt: "SEA" }),
          h('div', { className: "bebas", style: teamNameStyle(SEA) }, "SEAHAWKS")
        ),
        h('div', { className: "bebas", style: vsStyle }, "VS"),
        h('div', { style: teamBoxStyle },
          h('img', { src: "https://a.espncdn.com/i/teamlogos/nfl/500/ne.png", style: logoStyle(NER), alt: "NE" }),
          h('div', { className: "bebas", style: teamNameStyle(NER) }, "PATRIOTS")
        )
      );

      const form = h('div', { style: formBoxStyle },
        ...
        h('label', { style: labelStyle }, "ROOM CODE"),
        h('input', {
          value: room,
        ...
        h('button', { onClick: go, style: btnStyle }, "ENTER GAME →")
      );

      return h('div', { style: containerStyle },
        h('div', { style: bgStyle1 }),
        h('div', { style: bgStyle2 }),
        h('div', { style: contentWrapperStyle },
          timer,
          title,
          matchup,
          form
...
if start_idx != -1 and end_idx != -1:
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.writelines(lines)
    print("Rewrote SetupScreen in Pure JS.")
else:
