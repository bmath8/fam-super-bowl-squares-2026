...
new_setup_screen = """    // ── Setup Screen (Rewritten) ──────────────────────────────────
...
if start_idx != -1:
    for i in range(start_idx, len(lines)):
        if "function GameHeader" in line or "Compact Header" in lines[i]: # Compact Header comment usually precedes
             end_idx = i
             if "Compact Header" in lines[i]:
                 end_idx = i # Include the comment line in the next block, so end before it?
                 # Actually, we want to replace UP TO GameHeader.
                 # Let's verify clean cut.
             break
...
if start_idx != -1 and end_idx != -1:
    # Delete old range
    # But wait, end_idx is where GameHeader starts. We want to keep GameHeader.
    # So we replace lines[start_idx : end_idx]
    
    # Check if we should back up end_idx to remove empty lines?
    pass
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.writelines(lines)
    print("Rewrote SetupScreen successfully.")
else:
